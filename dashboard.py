...
def fetch_tickets_from_api(token: str, organizer_id: str,
                           event_id: str = "", include_cohosted: bool = False,
                           after: str = "", progress=None) -> pd.DataFrame:
...
with st.sidebar:
    ...
    with st.expander("Opções avançadas"):
        event_id_filter  = st.text_input("ID do Evento (opcional)")
        after_filter     = st.text_input("Somente após a data (ISO, opcional)", placeholder="2024-01-01")
        include_cohosted = st.checkbox("Incluir eventos co-organizados")

    fetch_clicked = st.button("🔄 Buscar Dados", use_container_width=True, type="primary")
        ...
        if not api_token or not organizer_id:
            ...
            try:
                with st.spinner("Conectando à API do Shotgun..."):
                    raw = fetch_tickets_from_api(
                        token=api_token,
                        organizer_id=organizer_id,
                        event_id=event_id_filter,
                        include_cohosted=include_cohosted,
                        after=after_filter,
                        progress=prog,
                    )
    ...
    uploaded = st.file_uploader("Exportação CSV", type="csv", label_visibility="collapsed")
    if uploaded:
    ...
    if "df" in st.session_state:
        ...
        st.divider()
        st.markdown("### Filtros")
        events = sorted(df_sidebar["event_name"].dropna().unique())
        sel_events = st.multiselect("Filtrar eventos", events, default=list(events))
        all_statuses = sorted(df_sidebar["ticket_status"].dropna().unique())
        sel_statuses = st.multiselect(
            "Status do ingresso", all_statuses,
...
if "df" not in st.session_state:
    ...
    with col:
        ...
        st.markdown("""
<div style='text-align:center; color:#9a9ab0; margin-bottom:2rem'>
Painel de análise de eventos conectado diretamente à API do Shotgun.
</div>
""", unsafe_allow_html=True)
        st.markdown("""
...
with tab_revenue:
    if "deal_price_brl" not in dff.columns:
        ...
        fig4 = px.pie(
            ...
            color_discrete_map={"Gratuito": "#636EFA", "Pago": "#EF553B"},
            hole=0.4,
        )
...
with tab_marketing:
    ...
    if "utm_source" in dff.columns:
        ...
        fig = px.bar(
            ...
            color="canal", color_discrete_sequence=COLORS,
            text="ingressos",
        )
...
with tab_audience:
    ...
    if "contact_newsletter_optin" in dff.columns:
        ...
        fig4 = px.pie(
            ...
            color_discrete_map={"Inscrito": "#00CC96", "Não Inscrito": "#EF553B"},
            hole=0.4,
        )
        fig4.update_traces(textinfo="percent+value")
        col_r2.plotly_chart(fig4, use_container_width=True)
    ...
    if len(sel_events) > 1:
        ...
        fig5 = px.bar(
            ...
            title="Participantes Recorrentes (Eventos Frequentados)",
            color="participantes", color_continuous_scale="Greens",
            text="participantes",
        )
